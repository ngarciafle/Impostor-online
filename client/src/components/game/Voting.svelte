...
{:else}
    ...
    <div class="flex flex-col md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <button class="aspect-4/3 bg-amber-100 shadow shadow-blue-700 w-7" disabled={voted} on:click={() => { selectedPlayer = player.name; voted = true; sendVote() }}>{player.name}</button>
        {/each}
        
        <button class="aspect-4/3 bg-blue-300 shadow shadow-orange-950 w-7" disabled={voted} on:click={() => {voted = true; sendVote()}}>Saltar</button>
